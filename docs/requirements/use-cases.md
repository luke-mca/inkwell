...
- **Primary Actor:** Visitor (an unauthenticated person)
- **Preconditions:** The visitor does not have an Inkwell account.
- **Postconditions:** A `User` record exists with a hashed password, and the account is pending verification.

### Main Success Scenario

1. The visitor submits an email, a display name, and a password.
2. The system validates the email format and the password strength.
3. The system confirms the email is not already registered.
4. The system hashes the password and sends a verification message to the submitted email address.

### Extensions

- **3a.** The email is already registered: the system responds with the same generic "check your email to continue" outcome it returns for a new address, so the response does not reveal whether an account exists. The existing account owner receives an email saying someone attempted to register with their address.
- **2a.** The password does not meet the strength policy: the system rejects the registration and states the policy, without storing anything.
- **2b.** The email is malformed: the system rejects the registration before touching the data store.
